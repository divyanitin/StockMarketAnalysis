...
# on value indiactes the same plot as the candle chart plot.
...
#The function eval.stats() uses two other functions to collect the precision and recall of the signals, and several economic evaluation metrics
eval.stats <- function(form,train,test,preds,b.t=0.1,s.t=-0.1,...) {
  # Signals evaluation
  tgtName <- all.vars(form)[1]
  test[,tgtName] <- trading.signals(test[,tgtName],b.t,s.t)
  #Function sigs.PR() receives as arguments the predicted and true signals, and calculates precision and recall for the sell, buy, and sell+buy signals
...
```{r}
#The functions single(), slide(), and grow() are called from the Monte Carlo routines with the proper parameters filled in so that we obtain the models we want to compare
